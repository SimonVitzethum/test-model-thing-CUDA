...
Every command-line program also exists in Zig; the kernels stay in `src/*.cu`
and are reached through the C API in `src/capi.h`. `make check-zig` compares
both builds on the same inputs, where all outputs are byte-identical except on
the MoE and trace paths, whose atomics make any two runs differ slightly.

```sh
zig build                 # zig-out/bin: train, sample, chat, gradcheck,
                          #              kgtrain, dialogprep, kgprep
...
Interactive demo: `kgtrain ask` indexes all nodes once, then for each question
shows the best-matching nodes, the loaded facts and the answer, generated byte
by byte:

```text
$ ./kgtrain ask s2.ckpt nodes=kg_nodes.tsv "What is the capital of Japan?"
retrieved: Japan (0.903), Jalapa (0.783), Empire of Japan (0.739), Lipa (0.663), Jinan (0.660)
memory:    Japan: capital: Tokyo; currency: yen; official language: Japanese; continent: Asia;
answer:    Tokyo
```

Without a question argument it reads questions from stdin. Only the trained
question forms and entities in the graph work.
