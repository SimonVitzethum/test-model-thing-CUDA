...
inline void linear_fwd(int M, int N, int K, const bf16* X, const bf16* W,
                       bf16* Y) {
    // Yt(N,M) = Wc(N,K) @ Xc(K,M), beide OP_N auf Row-Aliasen.
    float al = 1.0f, be = 0.0f;
    cublasStatus_t s = cublasGemmEx(
        cublas_handle(), CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &al,
        W, CUDA_R_16BF, N, X, CUDA_R_16BF, K, &be, Y, CUDA_R_16BF, N,
...
inline void linear_dW(int M, int N, int K, const bf16* dY, const bf16* X,
    ...
    cublasStatus_t s = cublasGemmEx(
        ...
        X, CUDA_R_16BF, K, dY, CUDA_R_16BF, N, &be, dW, CUDA_R_32F, K,
        CUBLAS_COMPUTE_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
